...
const extValueToSelection = (data: Object = {}): TreeSelection => {
	...
	for (const [name, attr] of Object.entries(data)) {
		...
		selection[name] = node as TreeAttr;
	}
...
const TreesOverride = (props: Props) => {
	props = merge({
		enabled: true,
		selection: {},
		onChange: () => {
	...
	const [selection, setSelection] = React.useState<TreeSelection>(props.selection as TreeSelection);
	...
	React.useEffect(() => {
		...
		if (isObject(extValue)) {
			setEnabled(true);
			setSelection(extValueToSelection(extValue as TreeAttr));
		}
	}, [environment]);
					...
					<Form.Group as={Row} className={cn('mb-2', {'text-muted': !isAltitudeEnabled})}>
						<Form.Label className="text-size-sm" column={true} sm="2">
							<Form.Check
								...
								className="text-size-xs"
								type="switch"
								id={`tree_override_altitude-switch-${nanoid(5)}`}
								label={
					...
					<Form.Group as={Row} className={cn('mb-2', {'text-muted': !isHumidityEnabled})}>
						<Form.Label className="text-size-sm" column={true} sm="2">
							<Form.Check
								...
								className="text-size-xs"
								type="switch"
								id={`tree_override_humidity-switch-${nanoid(5)}`}
								label={<span style={{textDecoration: 'underline dotted'}}
														 title="The min and max humidity at which this tree object is placed,
