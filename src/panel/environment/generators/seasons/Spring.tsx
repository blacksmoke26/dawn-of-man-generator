...
// parsers
import {toSeasonSpringTemplate} from '~/utils/parser/environment/templates';
...
const Spring = (props: Props) => {
  ...
  React.useEffect(() => {
    ...
    invokeHandler(props, 'onTemplate', toSeasonSpringTemplate(valuer.data, disabled));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="ml-1 pt-3" style={{minHeight: 350}}>
      ...
      <SeasonAttributeSlider
        disabled={disabled}
        value={valuer.data.fishBoost}
        onChange={val => valuer.set('fishBoost', val)}
        allowNumberInput
        caption="Fish Boost"
        title="Fish in banks will replenish by this amount at the beginning of this season."
        allowRestore allowShuffle allowMin allowMax
        onShuffle={() => valuer.set('fishBoost', random.randomSeasonVeryWindyChance())}
        onRestore={() => valuer.set('fishBoost', DEFAULT_VALUES.fishBoost)}>
        <Slider
          disabled={disabled}
          min={Defaults.SEASON_FISH_BOOST_MIN} max={Defaults.SEASON_FISH_BOOST_MAX}
          step={0.01} value={valuer.data.fishBoost}
          onChange={v => valuer.set('fishBoost', v)}/>
      </SeasonAttributeSlider>
      ...
      <div className="mt-3 text-right">
        <ButtonGroup>
          <LinkButton
            ...
            onClick={() => valuer.setAll(randomSeasonSpring())}>
